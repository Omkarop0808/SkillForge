...
class RequestCore:
    ...
    def syncPostRequest(self) -> httpx.Response:
        return httpx.post(
            ...
            proxy=self.proxy.get("https://") or self.proxy.get("http://") or None
        )

    async def asyncPostRequest(self) -> httpx.Response:
        async with httpx.AsyncClient(proxy=self.proxy.get("https://") or self.proxy.get("http://") or None) as client:
    ...
    def syncGetRequest(self) -> httpx.Response:
        return httpx.get(self.url, headers={"User-Agent": userAgent}, timeout=self.timeout, cookies={'CONSENT': 'YES+1'}, proxy=self.proxy.get("https://") or self.proxy.get("http://") or None)
